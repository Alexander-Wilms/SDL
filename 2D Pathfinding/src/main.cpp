...
int main(int, char**) {
	...
	color.g = 0;
	color.b = 0;
	putpixel(surface, 0, 0, color);
	putpixel(surface, 0, 1, color);
	putpixel(surface, 1, 1, color);
	putpixel(surface, 1, 0, color);

	color.r = 255;
	color.g = 0;
	color.b = 0;
	putpixel(surface, 359, 0, color);
	putpixel(surface, 359, 1, color);
	putpixel(surface, 358, 1, color);
	putpixel(surface, 358, 0, color);

	color.r = 0;
	color.g = 255;
	color.b = 0;
	putpixel(surface, 0, 359, color);
	putpixel(surface, 1, 359, color);
	putpixel(surface, 1, 358, color);
	putpixel(surface, 0, 358, color);

	color.r = 0;
	color.g = 0;
	color.b = 255;
	putpixel(surface, 359, 359, color);
	putpixel(surface, 358, 359, color);
	putpixel(surface, 359, 358, color);
	putpixel(surface, 358, 358, color);

	cout << (int) getpixel(surface, 0, 0).r << (int) getpixel(surface, 0, 0).g << (int) getpixel(surface, 0, 0).b  << endl;
	cout << (int) getpixel(surface, 359, 0).r << endl;
	cout << (int) getpixel(surface, 0, 359).g << endl;
	cout << (int) getpixel(surface, 359, 359).b << endl;
	update();
	...
	// Pfad suchen
	bool bFound = FindPath(nStartY, nStartX, nGoalY, nGoalX);

	Uint32 dwElapsed = SDL_GetTicks() - starttime;
	cout << "Die Suche dauerte: " << dwElapsed << "ms" << std::endl;
	cout << "Pfad gefunden: " << (bFound ? "ja" : "nein") << std::endl;
	...
	if (bFound) {
		...
		while (configuration_space_cells.at(x).at(y).nLastX != -1) // Bei -1 sind wir am Start
		{
			...
			configuration_space_colors.at(x).at(y).r = 255;
			configuration_space_colors.at(x).at(y).g = 0;
			configuration_space_colors.at(x).at(y).b = 0;
			Cell cell = configuration_space_cells.at(x).at(y);
			x = cell.nLastX;
			y = cell.nLastY;
			++length;
			update();
...
bool FindPath(int nStartX, int nStartY, int nGoalX, int nGoalY) {
	...
	configuration_space_cells[nStartY][nStartX].nLastX = -1;
