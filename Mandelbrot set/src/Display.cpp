...
const double MYBLA = 500;

Display::Display(double input) {
	rect = new SDL_Rect();

	vector<vector<double>> data(MYBLA, vector<double>(MYBLA));

	SDL_Init(SDL_INIT_VIDEO);

	window = SDL_CreateWindow("Window title", SDL_WINDOWPOS_CENTERED,
			SDL_WINDOWPOS_CENTERED, MYBLA, MYBLA, SDL_WINDOW_SHOWN);
